...
function createServer() {
  const server = new McpServer({ name: 'chougui-quantum-platform-public-docs', version: '1.0.0' });
  ...
  try {
    ...
    if (!transport && isInitializeRequest(req.body)) {
      transport = new StreamableHTTPServerTransport({
        ...
        onsessioninitialized: (id) => { if (transport) transports[id] = transport; }
      });
      transport.onclose = () => {
    ...
    if (!transport) {
      return res.status(400).json({ jsonrpc: '2.0', error: { code: -32000, message: 'Bad Request: initialize first or provide a valid MCP session id' }, id: null });
    }
...
app.get('/mcp', (_req, res) => res.status(405).set('Allow', 'POST').type('text/plain').send('Method Not Allowed'));

app.listen(PORT, () => console.log(`Chougui Quantum Platform public MCP server listening on :${PORT}`));
